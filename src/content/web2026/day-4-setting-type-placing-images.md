...
And if you want one nice flourish, pick a single technique from **[Jen Simmons' labs](https://labs.jensimmons.com/)** — a dropcap, columns, text wrapping a shape. One is elegant. Three is a mess.
...
- **`object-fit`** fits a picture into a fixed box: `cover` fills and crops, `contain` fits the whole thing inside. Use `cover` for photos of people — nobody wants their face stretched.
...
What we're deliberately **not** doing today: floats (an old way of doing layout; we'll wave from a distance) and `position: absolute` (the rule is *ask before you absolute* — nine times in ten flexbox does the job with far less pain, and I've lost whole evenings to a single stubborn absolutely-positioned image).
